...
async function loadData() {
  try {
    ...
    // 设置默认值
    const easyBaseEntry = chaptersData.value.find(chapter => chapter.section && chapter.section.startsWith('32-33'))
    const hardBaseEntry = chaptersData.value.find(chapter => chapter.section && chapter.section.startsWith('0-1'))
    const easyBaseEntry2 = chaptersData.value.find(chapter => chapter.section && chapter.section.startsWith('32-33'))
    if (easyBaseEntry) {
      // 对于级联选择器，我们需要设置完整的路径
